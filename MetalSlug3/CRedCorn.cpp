...
void CRedCorn::Initialize()
{
    ...
    m_pAnim->AddAnimation(L"SolDaeRokker_RedCorn_Left", pair<int, int>({ 0, 2 }));
    m_pAnim->AddAnimation(L"SolDaeRokker_RedCorn_Right", pair<int, int>({ 0, 2 }));
...
void CRedCorn::LateUpdate()
{
    __super::CheckOutOfWindow();
}

void CRedCorn::Render(HDC _hDC)
{
    m_pAnim->RenderAnimation(_hDC);
}

void CRedCorn::Release()
{
    SafeDelete<CCollider*>(m_pColBox);
    SafeDelete<CAnimation*>(m_pAnim);
}

void CRedCorn::OnCollision(CGameObject* _pCol, Vector2 _vColSize, COLLISION_COL_FLAG _eFlag)
{
...
void CRedCorn::SetBmpByAngle()
{
    ...
    if (m_fAngle <= 270.f)
    {
        m_pAnim->ChangeAnimation(L"SolDaeRokker_RedCorn_Left");
    ...
    {
        m_pAnim->ChangeAnimation(L"SolDaeRokker_RedCorn_Right");
